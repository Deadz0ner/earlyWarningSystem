...
In this project, an anchor is the large distributor/brand whose dealer network is being financed. So if a dealer belongs to ANCHOR_A vs ANCHOR_B, that means the dealer operates inside a different business environment:

different product demand stability,
different return/cancellation behavior,
different seasonality,
different channel discipline,
different portfolio quality overall

So when we say “Anchor B dealers are 1.3x more likely to default.
We are saying:
“As a portfolio-level modeling assumption, dealers under Anchor B come from a slightly riskier ecosystem, so we increase their baseline default probability by 30%.”
